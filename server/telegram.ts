...
async function getAgentResponse(chatId: string, userMessage: string): Promise<any> {
  ...
  try {
    ...
    if (!rawContent.trim()) {
      console.error("[Telegram] AI returned empty response");
      return { reply: "I'm here. What do you need?", action: "chat" };
    }
    ...
    if (jsonMatch) {
      try {
        ...
        console.error("[Telegram] JSON parse failed, using raw text:", parseErr.message);
        return { reply: cleaned, action: "chat" };
      }
    }

    return { reply: cleaned, action: "chat" };
  } catch (err: any) {
    console.error("[Telegram] getAgentResponse API error:", err.message || err);
    return { reply: "Connection issue — try again in a moment.", action: "chat" };
  }
}
  ...
  try {
    ...
    bot.on("message", async (msg: any) => {
      ...
      try {
        ...
        const replyText = response.reply || response.result || "I'm here.";
        ...
        try {
          await bot.sendMessage(chatId, "Something went wrong internally. Try again.");
        } catch (sendErr: any) {
          console.error("[Telegram] Failed to send error message:", sendErr.message || sendErr);
        }
      }
    });
  ...
  executeTask(task, async (taskId, step, message, status) => {
    try {
      if (step % 3 === 0 || status === "completed" || status === "failure") {
        const cleanMsg = truncateAtSentence(message, 300);
        await bot.sendMessage(chatId, `[Step ${step}] ${cleanMsg}`);
      }
    } catch (err: any) {
    ...
    if (completed) {
      ...
      if (completed.status === "completed") {
        const resultText = truncateAtSentence(completed.result || "Done.", 3000);
        statusMsg = `Task #${task.id} done!\n\n${resultText}\n\nTokens: ${completed.totalTokens} | Cost: $${completed.totalCostUsd}`;
      } else {
        statusMsg = `Task #${task.id} ${completed.status}: ${truncateAtSentence(completed.error || "Unknown error", 500)}`;
      }
      try {
